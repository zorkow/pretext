function init() {
    const codeSelector = 'code[role="application"][data-shellac]';
    const nav = '[role="application"][data-shellac],[role="group"],[role="treeitem"]';
    ...
    function click(snippet, e) {
        ...
        if (snippet.contains(clicked)) {
            const prev = snippet.querySelector('[tabindex="0"][role="group"],[tabindex="0"][role="treeitem"]');
            if (prev) {
